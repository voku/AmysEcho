...
export class PerformanceOptimizer {
    constructor() {
        ...
        this.targetFrameRate = 30; // Target FPS
        this.adaptiveFrameSkipping = false;
        // Frame skipping configuration
        this.skipFrameCount = 0;
        this.MAX_SKIP_FRAMES = 3; // Maximum consecutive frames to skip
        this.PROCESSING_TIME_THRESHOLD = 50; // ms - if processing takes longer, consider skipping
        // Landmark change tracking for overlay optimization
        this.lastLandmarksSignature = '';
        this.landmarkChangeThreshold = 0.01; // Minimum change to trigger redraw
    }
    /**
    ...
    calculateLandmarkChange(currentLandmarks) {
        if (!currentLandmarks || currentLandmarks.length === 0)
            return 0;
        if (!this.lastLandmarksSignature)
            return 1.0; // First time, consider it a change
        const currentSignature = this.generateLandmarksSignature(currentLandmarks);
        if (currentSignature === this.lastLandmarksSignature)
            return 0;
