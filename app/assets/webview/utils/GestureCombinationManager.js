...
export class GestureCombinationManager {
    constructor() {
        ...
        this.MAX_SEQUENCE_TIME = 5000; // 5 seconds max between gestures
        this.MIN_SEQUENCE_TIME = 200; // 200ms min between gestures
    ...
    checkCombination(sequence) {
        ...
        const now = Date.now();
        const recentGestures = this.gestureHistory.filter(h => now - h.timestamp <= timeWindow);
    ...
    cleanOldGestures() {
        ...
        this.gestureHistory = this.gestureHistory.filter(h => now - h.timestamp <= maxAge);
    }
    /**
