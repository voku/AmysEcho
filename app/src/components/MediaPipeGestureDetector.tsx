...
import { getCachedMlpModel } from '../services/dgsModelClient';
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const b64 = await getCachedMlpModel(pid ?? undefined);
        if (b64 && webviewRef.current && !mlpLoadedRef.current) {
          const safe = b64.replace(/\\/g, "\\\\").replace(/`/g, "\``").replace(/[\u2028\u2029]/g, '');
          webviewRef.current.injectJavaScript(`try{window.__setMlpModelB64 && window.__setMlpModelB64(\`${safe}\`);}catch(e){}`);
...
<head>
  ...
  <script>
    ...
    (function(){
      ...
      // Expose hooks for native side and predictor usage
        ...
        if (gestureRecognizer && video.currentTime > 0 && !video.paused && !video.ended) {
          if (lastVideoTime !== video.currentTime) { // Only process if video frame has changed
            ...
            if (window.__mlpPredict) {
              const mlpResult = window.__mlpPredict(allLandmarks);
