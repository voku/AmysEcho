/**
 * Validation utilities for Amy's Echo app
...
export const commonValidationRules = {
  ...
  array: (fieldName: string, minLength?: number, maxLength?: number): ValidationRule<any[]> => ({
  ...
  landmarkData: (fieldName: string): ValidationRule<any> => ({
...
export function validateGestureData(data: {
  ...
  const rules: ValidationRule[] = [
    commonValidationRules.gesture('gesture'),
    commonValidationRules.confidence('confidence'),
    {
    ...
    commonValidationRules.number('timestamp', 0)
  ];
...
export function validateApiResponse<T>(
  ...
  fieldValidators: Partial<Record<keyof T, ValidationRule>> = {}
): ValidationResult {
  const errors: string[] = [];
  const warnings: string[] = [];
  ...
  if (data && typeof data === 'object') {
    for (const field of requiredFields) {
      const present = (data as any)[field as any] !== undefined;
      if (!present) {
  ...
  for (const [field, validator] of Object.entries(fieldValidators) as [keyof T, ValidationRule][]) {
    if (data && typeof data === 'object' && (data as any)[field as any] !== undefined) {
      const fieldValue = (data as any)[field as any];
...
export function validateTrainingSample(sample: {
  ...
  const rules: ValidationRule[] = [
    commonValidationRules.required('gestureId'),
    commonValidationRules.gesture('gestureId'),
    commonValidationRules.required('landmarkData'),
    commonValidationRules.landmarkData('landmarkData'),
    {
      name: 'frameMetadata_valid',
      validate: (value) => !value.frameMetadata || commonValidationRules.object('frameMetadata').validate(value.frameMetadata),
...
export function validateProfile(profile: {
  ...
  const rules: ValidationRule[] = [
    commonValidationRules.required('id'),
    commonValidationRules.string('id', 1, 50),
    commonValidationRules.required('name'),
    commonValidationRules.string('name', 1, 100),
    {
      name: 'settings_valid',
      validate: (value) => !value.settings || commonValidationRules.object('settings').validate(value.settings),
