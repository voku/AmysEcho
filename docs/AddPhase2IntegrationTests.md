...
We will use React Testing Library with Vitest for the webapp to write our tests. Mock external dependencies (camera permissions, network uploads, etc.) so the suites remain deterministic.
...
3.  **Use Mocks for External Dependencies:**
    *   Use Vitest's mocking capabilities to mock any external dependencies.
