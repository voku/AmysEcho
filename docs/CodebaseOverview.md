...
Runtime enforcement of this budget is handled by the `AdaptivePerformanceManager` in `app/src/services/AdaptivePerformanceManager.ts`, which maintains a steady frame rate regardless of battery level or device thermal state to prioritize Amy's communication needs.
