...
| Post-training cadence (reconciliation, retention, summaries) | ML/Platform owner | Backend on-call | `post-training-cadence` reports + `/api/v1/train-status/cadence/latest` | 60 minutes |
...
6. If the issue is training-pipeline related, attach the latest post-training cadence summary before deciding on retry vs rollback.
