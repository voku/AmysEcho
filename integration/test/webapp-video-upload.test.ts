...
const LEFT_HAND_END = HAND_LANDMARK_COUNT;
const RIGHT_HAND_END = HAND_LANDMARK_COUNT * 2;
const POSE_LANDMARKS_START = RIGHT_HAND_END;
const POSE_LANDMARKS_END = 75;
const FACE_LANDMARKS_START = POSE_LANDMARKS_END;
const FACE_LANDMARKS_END = 543;
...
async function waitForTrainingCompletion(pollUrl: string) {
  ...
  while (Date.now() - start <= timeoutMs) {
    ...
    if (statusResp.status >= 400 && statusResp.status < 500) {
      const isTransient = statusResp.status === 404 || statusResp.status === 429;
      if (!isTransient) {
        const body = await statusResp.text().catch(() => 'unable to read response body');
        assert.fail(`Training poll returned non-retryable client error ${statusResp.status} for ${pollUrl}: ${body}`);
      }
    }
...
function parseLandmarks(frame: RepoLandmarkFrame) {
  ...
  const leftHand = points.slice(0, LEFT_HAND_END);
  const rightHand = points.slice(LEFT_HAND_END, RIGHT_HAND_END);
  const poseLandmarks = points.slice(POSE_LANDMARKS_START, POSE_LANDMARKS_END);
  const faceLandmarks = points.slice(FACE_LANDMARKS_START, FACE_LANDMARKS_END);

  return { leftHand, rightHand, poseLandmarks, faceLandmarks };
}
...
test('gesture detection works with downloaded profile model after training', async () => {
  const win = globalThis as any;
  const hasOwn = (k: string) => Object.prototype.hasOwnProperty.call(win, k);
  const hadWindow = hasOwn('window');
  const hadFflate = hasOwn('fflate');
  const hadReactNativeWebView = hasOwn('ReactNativeWebView');
  const hadNavigator = hasOwn('navigator');
  const hadLocalStorage = hasOwn('localStorage');
  ...
  try {
    ...
    const restore = (key: string, existed: boolean, value: unknown) => {
      if (existed) win[key] = value;
      else Reflect.deleteProperty(win, key);
    };
    restore('window', hadWindow, originalWindow);
    restore('fflate', hadFflate, originalFflate);
    restore('ReactNativeWebView', hadReactNativeWebView, originalReactNativeWebView);
    restore('navigator', hadNavigator, originalNavigator);
    restore('localStorage', hadLocalStorage, originalLocalStorage);
  }
});