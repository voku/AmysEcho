...
export async function handlePasswordResetRequest(
	...
	try {
		...
		return res.status(202).json({
			message:
				"Wenn ein Konto existiert, wurde eine E-Mail mit einem Reset-Code gesendet.",
		});
	} catch (error) {
...
export async function handlePasswordResetConfirm(
	...
	if (!parsed.success) {
		return res
			.status(400)
			.json({
				error: "E-Mail-Adresse, Reset-Code und neues Passwort werden benötigt.",
			});
	}
	...
	try {
		...
		if (!updatedUser) {
			return res
				.status(400)
				.json({ error: "Ungültiger oder abgelaufener Reset-Code." });
		}

		logger.info("Password reset confirmed", { userId: updatedUser.id });
