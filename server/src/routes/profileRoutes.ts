...
async function mergeProfileDirectories(sourceId: string, targetId: string): Promise<void> {
...
export function registerProfileRoutes(app: Express, deps: ProfileRouteDeps): void {
  ...
  app.post('/api/v1/profiles', authMiddleware, async (req, res) => {
    ...
    if (!parsed.success) {
      return res.status(400).json({ error: 'Profilinformationen fehlen oder sind ungültig.' });
    }
    ...
    try {
      ...
      return res.status(500).json({ error: 'Profil konnte nicht erstellt werden.' });
    }
  });
  ...
  app.post('/api/v1/profiles/:id/merge', authMiddleware, async (req, res) => {
    ...
    if (!parsed.success) {
      return res.status(400).json({ error: 'Zusammenführung benötigt eine gültige Quell-ID.' });
    }
    ...
    if (target.id === source.id) {
      return res.status(400).json({ error: 'Quellprofil und Zielprofil dürfen nicht identisch sein.' });
    }
    ...
    try {
      ...
      return res.status(500).json({ error: 'Profile konnten nicht zusammengeführt werden.' });
    }
  });
  ...
  app.post('/api/v1/profiles/:id/share', authMiddleware, (req, res) => {
    ...
    return res.json({ token: token.token, expiresAt: token.expiresAt, permissions: token.permissions });
  });

  app.post('/api/v1/profiles/share/accept', authMiddleware, async (req, res) => {
    const parsed = AcceptShareSchema.safeParse(req.body);
    if (!parsed.success) {
      return res.status(400).json({ error: 'Freigabe-Token fehlt.' });
    }
    const token = redeemShareToken(registry, parsed.data.token, req.user?.id ?? 'unknown');
    if (!token) {
      return res.status(404).json({ error: 'Freigabe-Token ungültig oder abgelaufen.' });
    }
  ...
  app.post('/api/v1/profiles/:id/sync-token', authMiddleware, (req, res) => {
    ...
    const token = createSyncToken(registry, profile.id, { deviceId: parsed.data.deviceId });
    if (parsed.data.deviceId) {
      registerDevice(registry, profile.id, { deviceId: parsed.data.deviceId, name: parsed.data.deviceName });
    }
  ...
  app.post('/api/v1/profiles/sync', authMiddleware, async (req, res) => {
    ...
    const token = redeemSyncToken(registry, parsed.data.token, parsed.data.deviceId);
    if (!token) {
      return res.status(404).json({ error: 'Synchronisierungs-Token ungültig oder abgelaufen.' });
    }
    ...
    try {
      ...
      res.setHeader('Content-Disposition', `attachment; filename="profile_${profile.id}_sync.zip"`);
      res.setHeader('X-Profile-Checksum', checksum);
      return res.status(200).send(buffer);
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      logError('Profile sync export failed', { error: message });
      return res.status(500).json({ error: 'Synchronisierung fehlgeschlagen.' });
    }
  });
  ...
  app.post('/api/v1/profiles/:id/sync', authMiddleware, async (req, res) => {
    ...
    try {
      ...
      return res.status(500).json({ error: 'Synchronisierung konnte nicht eingespielt werden.' });
    }
  });
  ...
  app.post('/api/v1/profiles/:id/backup', authMiddleware, async (req, res) => {
    ...
    try {
      ...
      return res.status(500).json({ error: 'Backup konnte nicht erstellt werden.' });
    }
  });
  ...
  app.post('/api/v1/profiles/:id/restore', authMiddleware, async (req, res) => {
    ...
    try {
      ...
      return res.status(500).json({ error: 'Backup konnte nicht wiederhergestellt werden.' });
    }
  });
  ...
  app.delete('/api/v1/profiles/:id/data', authMiddleware, async (req, res) => {
    ...
    try {
      ...
      return res.status(500).json({ error: 'Profil-Daten konnten nicht gelöscht werden.' });
    }
  });
}
