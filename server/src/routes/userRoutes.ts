...
export function registerUserRoutes(app: import('express').Express, deps: UserRouteDeps) {
  app.put('/api/user/profile', deps.authMiddleware, async (req: Request, res: Response) => {
    const parsed = ProfileUpdateSchema.safeParse(req.body);
    if (!parsed.success) {
      return res.status(400).json({ error: 'Ungültige Profildaten.', details: parsed.error.flatten() });
    }

    if (parsed.data.userId) {
      return res.status(403).json({ error: 'Änderungen für andere Konten sind nicht erlaubt.' });
    }
    ...
    try {
      ...
      return res.status(500).json({ error: 'Profilaktualisierung fehlgeschlagen.' });
    }
  });

  app.put('/api/user/password', deps.authMiddleware, async (req: Request, res: Response) => {
    const parsed = PasswordUpdateSchema.safeParse(req.body);
    if (!parsed.success) {
      return res.status(400).json({ error: 'Ungültige Passwortdaten.', details: parsed.error.flatten() });
    }
    ...
    try {
      const updated = await withFileLock(deps.dbFilePath, async () => {
        ...
        user.passwordHash = await AuthService.hashPassword(parsed.data.newPassword);
        await saveDatabase(deps.db, deps.dbFilePath);
        return user;
      });
