...
export async function deleteProfileTrainingData(
	profileId: string,
): Promise<void> {
	...
	const remainingSamples = samples.samples.filter(
		(sample) => sample.profileId !== profileId,
	);
	await saveDgsSamples({ samples: remainingSamples });

	const customSigns = await loadCustomSigns();
	const remainingSigns = customSigns.signs.filter(
		(sign) => sign.profileId !== profileId,
	);
...
export async function buildProfileExportArchive(
	...
	const manifest = filterManifestForProfile(
...
export async function restoreProfileFromArchive(
	...
	const payload = JSON.parse(
		profileEntry.getData().toString("utf8"),
	) as ProfileExportPayload;
	...
	const manifestEntries = existingManifest.entries.filter(
		(entry) => entry.profileId !== profileId,
	);
	const newManifest = {
		entries: [...manifestEntries, ...payload.trainingManifest.entries],
	};

	const existingSamples = await loadDgsSamples();
	const sampleEntries = existingSamples.samples.filter(
		(sample) => sample.profileId !== profileId,
	);
	const newSamples = {
		samples: [...sampleEntries, ...payload.dgsSamples.samples],
	};

	const existingSigns = await loadCustomSigns();
	const restoredCustomSigns = payload.customSigns ?? { signs: [] };
	const remainingSigns = existingSigns.signs.filter(
		(sign) => sign.profileId !== profileId,
	);
	...
	const uploadsEntries = zip
		.getEntries()
		.filter((entry) => entry.entryName.startsWith("uploads/"));
	...
	const modelsEntries = zip
		.getEntries()
		.filter((entry) => entry.entryName.startsWith("models/"));
