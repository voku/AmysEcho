...
async function renameDirectory(baseDir: string, fromId: string, toId: string): Promise<void> {
...
export async function migrateProfileIds(
  db: Database,
  registryPath: string,
): Promise<{ registry: ProfileRegistry; mapping: ProfileIdMapping; migrated: boolean }> {
