...
function normalizeMetadata(input?: Partial<ProfileMetadata>): ProfileMetadata {
  return {
    ...
    birthDate: typeof input?.birthDate === 'string' ? input.birthDate : undefined,
    primaryLanguage: typeof input?.primaryLanguage === 'string' ? input.primaryLanguage : undefined,
...
function normalizeProfileRecord(record: ProfileRecord): ProfileRecord {
  return {
    ...
    devices: Array.isArray(record.devices) ? record.devices.filter(Boolean) : [],
    caregivers: Array.isArray(record.caregivers) ? record.caregivers.filter(Boolean) : [],
  };
}
...
export async function loadProfileRegistry(filePath: string): Promise<ProfileRegistry> {
  try {
    ...
    registry.updatedAt = typeof parsed.updatedAt === 'string' ? parsed.updatedAt : nowIso();
    registry.profiles = Array.isArray(parsed.profiles) ? parsed.profiles.map(normalizeProfileRecord) : [];
    registry.syncTokens = Array.isArray(parsed.syncTokens) ? parsed.syncTokens : [];
    registry.shareTokens = Array.isArray(parsed.shareTokens) ? parsed.shareTokens : [];
...
export async function saveProfileRegistry(filePath: string, registry: ProfileRegistry): Promise<void> {
  await fs.mkdir(path.dirname(filePath), { recursive: true });
  await atomicWriteJson(filePath, registry);
}

export function findProfileRecord(registry: ProfileRegistry, id: string): ProfileRecord | undefined {
...
export function updateProfileRecord(
  ...
  if (updates.metadata) {
    record.metadata = { ...record.metadata, ...normalizeMetadata(updates.metadata) };
  }
...
export function attachCaregiver(
  ...
  const record = findProfileRecord(registry, profileId);
  if (!record) return undefined;
  const existing = record.caregivers.find((c) => c.caregiverId === caregiver.caregiverId);
  const now = nowIso();
  if (existing) {
