...
export async function apiRequest<T = any>(
  ...
  const shouldRetryFn = (error: Error) => {
    ...
    if (!retryable) {
      logger.warn(`Not retrying ${method} ${url}: ${error.message}`, undefined, userId);
    }
    return retryable;
  };
...
export class ExternalApiClient {
  ...
  async get<T = any>(endpoint: string, options?: { headers?: Record<string, string>; userId?: string }): Promise<ApiResponse<T>> {
    return this.request<T>('GET', endpoint, options);
  }

  async post<T = any>(endpoint: string, body?: any, options?: { headers?: Record<string, string>; userId?: string }): Promise<ApiResponse<T>> {
    return this.request<T>('POST', endpoint, { ...options, body });
  }

  async put<T = any>(endpoint: string, body?: any, options?: { headers?: Record<string, string>; userId?: string }): Promise<ApiResponse<T>> {
    return this.request<T>('PUT', endpoint, { ...options, body });
  }

  async delete<T = any>(endpoint: string, options?: { headers?: Record<string, string>; userId?: string }): Promise<ApiResponse<T>> {
